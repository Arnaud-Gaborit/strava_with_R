...
# Récupération des traces de toutes les activités CAP
recup_traces_gps = function(liste_activites, nsimplification = 1) {
  
  # initialisationd de la table qui contiendra les résultats
  coord.df.tot <- data.frame()
  
  i = 0
  
  for (id_act in liste_activites) {
    i = i + 1
    print(i)
    # stream = get_streams(stoken, id_act, request = "activities", types = NULL,
    #                      resolution = NULL, series_type = NULL)
    
    stream = get_streams(stoken, id = id_act, types = list('distance', 'latlng'))
    
    coord.df <- get.coord.df.from.stream(stream)
    coord.df$id_act = id_act
    
    # si besoin de simplifier la trace
    coord.df = coord.df[sort(sample(nrow(coord.df), nrow(coord.df)/nsimplification)), ]
    
    coord.df.tot = rbind(coord.df.tot, coord.df)
  }
  return(coord.df.tot)
}


# Fonction calendrier sous ggplot
calendarHeat <- function(dates, 
  ...
  strava_rn <- c('#ffb084', '#000000') # rose à noir
  strava_on <- c('#fc5200', '#000000') # orange à noir
  strava_gn <- c('#CCCCCC', '#000000') # gris à noir
  strava_ro <- c('#ffb084', '#fc5200') # rose à orange
  ...
  print(cal.plot <- levelplot(value~woty*dotw | yr, data=caldat,
                              ...
                              strip=TRUE,
                              main = paste("Calendrier ", varname, sep = ""),
                              scales = list(
                                x = list(
                                  at= c(seq(2.9, 52, by=4.42)),
                                  #labels = month.abb,
                                  labels = c('Jan', 'Fev', 'Mar', 'Avr', 'Mai', 'Juin', 'Juil', 'Aou', 'Sep', 'Oct', 'Nov', 'Dec'),
